...
var a = 'ckwO6bJ9byAuO1aYnsR303owo795tbMK9LGGMjpU';

$.ajax({
	type : 'GET',
	data : {get_param: 'value'},
	dataType : 'jsonp',
	url : 'https://#'+a+':bibles.org/v2/versions/eng-GNTD/Gen/1.js',
...
$(document).foundation();
				...
				success:function(json){
					...
					if (json.type == 'verse'){
						...
						$.each(json.book, function(index, value) {
							output += '<div id="book"><h3>'+value.book_name+' '+value.chapter_nr+'</h3></div><br/><p class="'+direction+'">';
							$.each(value.chapter, function(index, value) {
								output += '  <span class="ltr text-gray">' +value.verse_nr+ '.</span>  ';
								output += value.verse;
								output += '<br/>';
							});
							output += '</p>';
						...
						var output = '<div id="book"><h3>'+json.book_name+' '+json.chapter_nr+'</h3></div><br/><p class="'+direction+'">';
						...
						var output = '';
						$.each(json.book, function(index, value) {
							output += '<div id="book"><h3>'+json.book_name+' '+value.chapter_nr+'</h3></div><br/><p class="'+direction+'">';
							$.each(value.chapter, function(index, value) {
								output += '  <span class="ltr text-gray">' +value.verse_nr+ '.</span>  ';
								output += value.verse;
								output += '<br/>';
  ...
  $("#oldTest").click(function(event) {
    event.preventDefault();
    searchScripture('oldTest');
  });

  $("#newTest").click(function(event) {
      event.preventDefault();
